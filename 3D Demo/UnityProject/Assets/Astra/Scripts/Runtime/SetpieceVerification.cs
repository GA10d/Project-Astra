...
namespace AstraCabin
{
    ...
    public sealed class SetpieceVerification : MonoBehaviour
    {
        ...
        IEnumerator VerifySevenToNine()
        {
            ...
            controller.SetPaused(false);
            yield return VerifyCabinWhaleViews();
            director.ResetToDefault(); yield return null;
            ...
            Check("Zero also removes every interior phasing whale", whales.CabinWhaleCount == 0 && whales.CabinWhales.Count == 0);
        ...
        static bool InsideCabin(Vector3 p)
        {
            return Mathf.Abs(p.x) < 1.12f && Mathf.Abs(p.z) < 1.25f && p.y > .32f && p.y < 2.25f;
        }

        void VerifyCabinWhalePaths()
        {
            Check("Phasing school adds at least eight small physical whales", whales.CabinWhaleCount >= 8 && whales.CabinWhales.Count == whales.CabinWhaleCount);
            Check("Large exterior whale migration remains intact", whales.WhaleCount == 7 && whales.FollowerCount == 120);
            Check("Phasing animals have no active colliders to intercept cabin clicks", whales.GeneratedRoot && !whales.GeneratedRoot.GetComponentsInChildren<Collider>(true).Any(c => c.enabled));
            int allSides = 0, throughRoutes = 0;
            bool continuous = true, finite = true, cameraClearance = true, matching = true, small = true;
            for (int i = 0; i < whales.CabinWhaleCount; i++)
            {
                Transform whale = whales.CabinWhales[i];
                matching &= whale && Vector3.Distance(whale.position, whales.SampleCabinWhalePosition(i, whales.Elapsed)) < .003f;
                small &= whale && whale.localScale.x >= .04f && whale.localScale.x <= .14f;
                int sides = 0; bool inside = false;
                Vector3 previous = whales.SampleCabinWhalePosition(i, 0);
                for (int step = 0; step <= 2400; step++)
                {
                    Vector3 p = whales.SampleCabinWhalePosition(i, step * .05f);
                    finite &= !float.IsNaN(p.x + p.y + p.z) && !float.IsInfinity(p.x + p.y + p.z);
                    continuous &= Vector3.Distance(previous, p) < .35f;
                    cameraClearance &= Vector3.Distance(p, controller.centerPosition) >= .30f && Vector3.Distance(p, controller.windowObservationPosition) >= .30f;
                    inside |= InsideCabin(p);
                    if (p.x > 1.45f) sides |= 1;
                    if (p.x < -1.45f) sides |= 2;
                    if (p.z > 1.65f) sides |= 4;
                    if (p.z < -1.65f) sides |= 8;
                    if (p.y > 2.6f) sides |= 16;
                    if (p.y < -.1f) sides |= 32;
                    previous = p;
                }
                allSides |= sides;
                bool opposite = (sides & 3) == 3 || (sides & 12) == 12 || (sides & 48) == 48;
                if (inside && opposite) throughRoutes++;
            }
            Check("Every small whale genuinely enters cabin and exits opposite sides", throughRoutes == whales.CabinWhaleCount, "through-routes=" + throughRoutes);
            Check("School crosses all four walls rather than only the window", (allSides & 15) == 15, "side-mask=" + allSides);
            Check("Whale paths remain continuous for two minutes without loop teleports", continuous && finite);
            Check("Whale paths keep clearance around center and near-window camera", cameraClearance);
            Check("Live whale positions match their sampled physical paths", matching);
            Check("Interior whales use smaller bodies than the exterior matriarch", small);
        }

        IEnumerator VerifyCabinWhaleViews()
        {
            string[] labels = { "A", "D", "C", "B" };
            for (int wall = 0; wall < 4; wall++)
            {
                controller.SnapToWall(wall); controller.SetLookForTest(Vector2.zero);
                yield return null;
                float deadline = whales.Elapsed + 12f;
                bool seen = false;
                while (!seen && whales.Elapsed < deadline)
                {
                    foreach (Transform whale in whales.CabinWhales)
                    {
                        if (!whale || !InsideCabin(whale.position)) continue;
                        Vector3 viewport = controller.viewCamera.WorldToViewportPoint(whale.position);
                        if (viewport.z > .38f && viewport.x > .16f && viewport.x < .84f && viewport.y > .13f && viewport.y < .87f) { seen = true; break; }
                    }
                    if (!seen) yield return null;
                }
                Check("Interior whale visibly crosses the " + labels[wall] + " wall view", seen);
                yield return Capture((29 + wall) + "_" + labels[wall] + "_Small_Whales_Inside_Cabin");
            }
            Vector3[] positions = whales.CabinWhales.Select(w => w.position).ToArray();
            controller.SetPaused(true); yield return new WaitForSecondsRealtime(.25f);
            Check("Pause freezes actual interior whale geometry", whales.CabinWhales.Select((w, i) => Vector3.Distance(w.position, positions[i])).All(d => d < .0001f));
            controller.SetPaused(false);
            yield return new WaitForSeconds(.5f);
            Check("Interior whale movement resumes after pause", whales.CabinWhales.Select((w, i) => Vector3.Distance(w.position, positions[i])).Any(d => d > .03f));
        }
