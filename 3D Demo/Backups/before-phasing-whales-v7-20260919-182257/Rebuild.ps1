...
if ($RegenerateModel) {
    ...
    & $Blender -b (Join-Path $demoRoot 'Source\Astra_Cabin.blend') --python (Join-Path $demoRoot 'Source\apply_materials.py')
    if ($LASTEXITCODE -ne 0) { throw 'Blender material refresh failed.' }
}
