...
namespace AstraCabin
{
    public sealed class SetpieceAudio : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            foreach(string name in new[]{"impact","explosion","laser","ufo","engine","warp","monster","hammer","success","alert","alarm","whale","echo","scanner","stamp","conveyor"})
        ...
        static AudioClip Synthesize(string name)
        {
            ...
            if(name=="whale") duration=4.2f;
            else if(name=="echo") duration=2.1f;
            else if(name=="scanner") duration=1.5f;
            else if(name=="stamp") duration=.52f;
            else if(name=="conveyor") duration=2.8f;
            ...
            for(int i=0;i<data.Length;i++)
            {
                ...
                switch(name)
                {
                    ...
                    case "whale":
                        float call=2*Mathf.PI*(71*t+8*t*t)+Mathf.Sin(t*2.7f)*1.8f;
                        value=(Mathf.Sin(call)*.35f+Mathf.Sin(call*2.013f)*.14f+Mathf.Sin(call*3.02f)*.055f)*Mathf.Pow(Mathf.Sin(u*Mathf.PI),1.4f); break;
                    case "echo":
                        float pulse=Mathf.Exp(-Mathf.Repeat(t,.52f)*9f)*Mathf.Sin(u*Mathf.PI);
                        value=(Mathf.Sin(t*1040)*.20f+Mathf.Sin(t*1083)*.12f+Mathf.Sin(t*203)*.15f)*pulse; break;
                    case "scanner": value=(Mathf.Sin(2*Mathf.PI*(480*t+260*t*t))*.23f+filtered*.08f)*Mathf.Sin(u*Mathf.PI); break;
                    case "stamp": value=(filtered*1.3f+Mathf.Sin(t*510)*.32f+Mathf.Sin(t*1700)*.14f)*Mathf.Exp(-u*10); break;
                    case "conveyor": value=(filtered*.6f+Mathf.Sin(t*194)*.13f+Mathf.Sin(t*388)*.07f)*(.7f+.3f*Mathf.Sin(t*14))*Mathf.Sin(u*Mathf.PI); break;
