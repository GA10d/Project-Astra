...
namespace AstraCabin
{
    ...
    public sealed class SetpieceDirector : MonoBehaviour
    {
        ...
        public bool TriggerMode(int mode)
        {
            ...
            Initialize();
            bool keepNear=controller.IsAtWindow && mode!=3 && mode!=0;
            ...
            if(keepNear) controller.EnterWindowView();
        ...
        void ClearCurrent()
        {
            ...
            combat.Clear(); visitors.Clear(); crimson.Clear(); eventAudio.StopAll();
        ...
        void SetAlert(string title,string detail)
        {
        ...
        void OnGUI()
        {
            ...
            GUI.Label(new Rect(width*.5f-380,28,760,27),"演出  [1] 爆星   [2] 访客   [3] 撞击   [4] 赤域   [5] 战场   [6] 补给   [0] 复位",small);
            if(ActiveMode!=0)
            {
