...
namespace AstraCabin
{
    ...
    public sealed class CabinController : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            viewCamera.transform.position = centerPosition;
            viewCamera.transform.rotation = baseRotation * Quaternion.Euler(-currentLook.y * mouseLookDegrees, currentLook.x * mouseLookDegrees, 0f);
            viewCamera.fieldOfView = fieldOfView;
            if (!automationMode && !IsPaused)
            {
                UpdateHover();
        ...
        {
            if (direction == 0 || IsTurning || IsPaused) return;
        ...
        {
            if (IsPaused || IsTurning || !hovered) return false;
            bool accepted = hovered.TryInteract();
            if (accepted) ShowNotice(hovered.title + (hovered.IsOn ? " / ON" : " / OFF"));
            return accepted;
        }
        ...
        private void OnGUI()
        {
            ...
            GUI.Label(new Rect(44f, 59f, 305f, 25f), wall, smallStyle);
            GUI.Label(new Rect(w - 283f, 29f, 254f, 25f), "PIGEON PROGRAM  //  20XX", smallStyle);

            Panel(new Rect(28f, h - 62f, 280f, 37f), panel);
            GUI.Label(new Rect(44f, h - 59f, 255f, 31f), "[ Q ]  左转      右转  [ E ]", smallStyle);
            Panel(new Rect(w - 255f, h - 62f, 227f, 37f), panel);
            GUI.Label(new Rect(w - 241f, h - 59f, 206f, 31f), "[ M ] 声音   [ ESC ] 暂停", smallStyle);
            ...
            if (hovered && !IsTurning && !IsPaused)
            {
                ...
                string hint = hovered.IsBusy ? "机械动作中…" : "鼠标左键  /  " + (hovered.IsOn ? "当前开启" : "当前关闭");
