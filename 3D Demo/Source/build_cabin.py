...
def gauge(u,h,d=.24,r=.11,label="BAR"):
    ...
    a=1.05
    # Keep each pointer as an independently editable/exported mesh. Its origin
    # is the spindle, not the centre of the cabin or the centre of the rod.
    name="GAUGE_"+WALL+"_"+label
    needle=rod(name,(u,h,d+.052),(u+r*.6*math.cos(a),h+r*.6*math.sin(a),d+.052),.003,"RedPaint",6)
    pivot=wp(u,h,d+.052)
    needle.data.transform(Matrix.Translation(-pivot))
    needle.location=pivot
    needle["astra_keep_name"]=True
    needle["astra_gauge"]=label
    needle["astra_pivot"]=list(pivot)
    world_axis={"A":[0,0,1],"B":[-1,0,0],"C":[0,0,-1],"D":[1,0,0]}[WALL]
    GAUGES.append({"name":name,"wall":WALL,"label":label,
        "blender_position":list(pivot),"unity_expected_position":world_to_unity(pivot),
        "unity_world_axis":world_axis,"maximum_swing_degrees":2.4})
